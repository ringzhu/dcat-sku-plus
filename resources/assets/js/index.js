(function () {
    ...
    SKU.prototype.init = function () {
        ...
        // 绑定添加属性名事件
        _this.wrap.find('.sku_attr_key_val').on('click', '.Js_add_attr_name', function () {
        ...
        if (old_val) {
            ...
            _this.wrap.find('.sku_attr_key_val tbody').append(_this.getHtml('','add'));
            // 生成具体的SKU配置表单
    ...
    SKU.prototype.getHtml = function (innerHtml = '',type = 'remove') {
        let _this = this;
        ...
        if(type == 'remove'){
            html += '</select><input type="text" class="form-control input_attr_name"></td><td><div class="sku_attr_val_wrap">' +
                ...
                '</div></td><td style="display: flex;justify-content: space-around;align-items: center;border-bottom: 0;"><span class="btn btn-primary Js_add_attr_name">添加</span><span class="btn btn-default Js_remove_attr_name">移除</span></td></tr>';
        }else {
            html += '</select><input type="text" class="form-control input_attr_name"></td><td><div class="sku_attr_val_wrap">' +
                (innerHtml.length > 0 ? innerHtml : this.getAttributeHtml('input', this.skuAttr)) +
                '</div></td><td style="display: flex;justify-content: space-around;align-items: center;border-bottom: 0;"><span class="btn btn-primary Js_add_attr_name">添加</span></td></tr>';
        }
        return html;
    };
